...
Die eigentliche Logik zur Steuerung der Lichter kann auf einem \textit{\gls{rpi}}
...
ZigBee-Chip, mit dem sie Teil eines ZigBee-Netzwerks werden können. In diesem Netzwerk
fungieren sie als \emph{End Device}. Über das ZigBee \emph{Light Link}-Protokoll können
die Lampen angesprochen werden und bestimmte Einstellungen wie Helligkeit und Farbe
eingestellt werden.
...
Der ZigBee Controller \emph{RaspBee} ist die eigentliche Funkeinheit. Sie stellt eine rohe
...
aufsteckbare Erweiterungsplatine mit Funkmodul für \emph{\gls{rpi}}, und zum
anderen dem \emph{\gls{rpi}} selber. Der \emph{\gls{rpi}}, ein
...
\noindent
Das \emph{RaspBee} ist an diese GPIO Pins angepasst und wird dadurch mit Strom versorgt.
Außerdem werden die \emph{UART}-Pins zur seriellen Kommunikation mit einem Treiber, der
auf dem \emph{\gls{rpi}} betrieben wird, verwendet.

\subsection{Lichtsteuerung}

Die Lichtsteuerung soll als hardwarenahes Backend dienen. Diese Software, die auf
dem \emph{\gls{rpi}} betrieben wird, kommuniziert über die serielle
...
Die API ist eine weitere Software auf dem \emph{\gls{rpi}}. Sie verwendet
...
Das im \emph{\gls{rpi}} Bundle vorinstallierte System ist NOOBS. Dieses System dient
als Platform um das eigentliche System zu installieren. Es stellt eine grafische
Oberfläche auf der HDMI-Schnittstelle bereit auf der das über das Internet zu
installierende Betriebssystem ausgewählt werden kann.

Da die deCONZ-Software nur in Binärform vorliegt muss ein daran angepasstes
Betriebssystem installiert werden. Raspbian\footnote{\url{https://www.raspbian.org/}},
eine für das \emph{\gls{rpi}} angepasste Debian Version, ist in der Version Wheezy
zur deCONZ-Software kompatibel. Weiterhin ist Raspbian eines der verbreitetsten
Systeme für den \emph{\gls{rpi}}.
...
Der \emph{\gls{rpi}} bekommt automatisch von einem im Netzwerk verbundenen DHCP-
...
deCONZ stellt zwei Bedienoberflächen bereit. Eine Oberfläche wird als
Desktop-Applikation angezeigt. Dort kann sich auf einer hardwarenahen Ebene verfügbare
